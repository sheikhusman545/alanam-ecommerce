...
export class PaymentPage implements OnInit {
  ...
  async onSubmit() {
    if (this.orderForm.valid) {
      await this.presentLoading('Wait...');
  ...
  processSelectedPayment(paymentMethod: any) {
    //if PaymentMethodId is not 6 then navigate to paynow page
    ...
    // // Example: Send the selected payment method to the backend or handle further logic
    // this.paymentService.getSessionId(paymentMethod.PaymentMethodEn).subscribe(
    //   (response: any) => {
    //     console.log('Session ID:', response.sessionID);
    //     // Proceed with payment processing using session ID
    //   },
    //   (error:any) => {
